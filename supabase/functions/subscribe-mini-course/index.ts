...
// "Hannah Morris" segment — triggers the 10-email mini course flow
const SEGMENT_ID = '68a227b3d77f9f886c6cc457';
...
Deno.serve(async (req) => {
  ...
  if (!email || !email.includes('@')) return new Response(JSON.stringify({ error: 'Email required' }), {
  ...
  const upsertRes = await fetch('https://api.flodesk.com/v1/subscribers', {
    ...
    body: JSON.stringify({ email, status: 'active', custom_fields: { utm_source: utm_source ?? 'direct' } }),
  });
